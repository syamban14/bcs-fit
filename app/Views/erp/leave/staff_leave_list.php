...
if($user_info['user_type'] == 'staff'){
	...
	$leave_types = $ConstantsModel->where('type','leave_type')->orderBy('constants_id', 'ASC')->findAll();
	$total_leave = $LeaveModel->orderBy('leave_id', 'ASC')->countAllResults();
	$leave_pending = $LeaveModel->where('status', 1)->countAllResults();
	$total_accepted = $LeaveModel->where('status', 2)->countAllResults();
	$total_rejected = $LeaveModel->where('status', 3)->countAllResults();
}
?>
        ...
        <div id="accordion">
          ...
          <div class="card-body">
            <div class="row">
              ...
              <?php $staff_info = $UsersModel->where('user_type','staff')->findAll();?>
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <select class="form-control" name="employee_id" data-plugin="select_hrm" data-placeholder="<?= lang('Dashboard.dashboard_employee');?>">
                    <?php foreach($staff_info as $staff) { 
                            $nama=$HrisModel->where('payroll_id',$staff['payroll_id'])->first();
                            $name=$nama['nama_karyawan'];
                      ?>
                    <option value="<?= $staff['user_id']?>">
                    <?= $staff['payroll_id'].' - '.$name ?> 
                    </option>
                    <?php } ?>
                  </select>
                </div>
...
<div class="card user-profile-list <?php echo $get_animate;?>">
  <div class="card-header">
    ...
    <?php if(in_array('leave3',staff_role_resource()) || $user_info['user_type'] == 'company') {  
            if($user_info['user_id']=='58' || $user_info['user_id']=='59' || $user_info['user_id']=='60'){
              echo'';
            }else{?>
    <div class="card-header-left"> <a  data-toggle="collapse" href="#add_form" aria-expanded="false" class="collapsed btn waves-effect waves-light btn-primary btn-sm m-0"> <i data-feather="plus"></i>
