...
if( !function_exists('attendance_time_checks') ){
	...
	function attendance_time_checks(){
		...
		$yesterday2 = date('Y-m-d',strtotime("-1 days")). "\n";
		$today_date = date('Y-m-d');
		$UsersModel = new \App\Models\UsersModel();
		// $TimesheetModel = new \App\Models\TimesheetModel();
		$TimesheetModel = new \App\Models\TimesheetyesterdayModel();
		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();

		// $get_data = $TimesheetModel->where('company_id',$user_info['company_id'])->where('employee_id',$usession['sup_user_id'])->orderBy('time_attendance_id', 'DESC')->countAllResults();
	//  $get_data = $TimesheetModel->where('company_id',$user_info['company_id'])->where('employee_id',$usession['sup_user_id'])->where('attendance_date',$today_date)->where('clock_out','')->orderBy('time_attendance_id', 'DESC')->countAllResults();
		    // Cari data kehadiran dengan clock in di hari sebelumnya atau hari ini yang belum diikuti clock out
		$get_data = $TimesheetModel->where('company_id', $user_info['company_id'])
									->where('employee_id', $usession['sup_user_id'])
									->where("(attendance_date = '{$yesterday_date}' OR attendance_date = '{$today_date}')", null, false)
									->where('clock_out', '')
