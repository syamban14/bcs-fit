...
class Leave extends BaseController {
	...
	public function leave_list() {
		...
		$get_data = $LeaveModel->orderBy('leave_id', 'ASC')->findAll();
		...
		$data = array();		
          foreach($get_data as $r) {			  
			...
			$duration = set_date_format($r['from_date']).' Sampai '.set_date_format($r['to_date']);
			$total_days = $idays;
			// leave status
	...
	// record list dashboard
	public function leave_list_dash() {
		...
		$HrisModel = new HrisModel();
		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
	 	$get_data = $LeaveModel->where('company_id',$usession['sup_user_id'])->where('from_date',date('Y-m-d'))->orderBy('leave_id', 'ASC')->findAll();
	...
	{		
		...
		$data['title'] = lang('Dashboard.xin_manage_leaves').' | '.$xin_system['application_name'];
		$data['path_url'] = 'leave_dash';
		$data['breadcrumbs'] = lang('Dashboard.xin_manage_leaves').$user_id;

		$data['subview'] = view('erp/leave/staff_leave_list_dash', $data);
		return view('erp/layout/layout_main', $data); //page load
	}
}
