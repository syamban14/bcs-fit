...
use App\Models\TimesheetWorkModel;
use App\Models\TimesheetijinB3Model;
...
class Timesheet extends BaseController { 
	...
	public function attendance_list() {
		...
		foreach($get_data as $r) {  
			...
			if($check_in_row > 0){
				...
				// $attendance = $TimesheetModel->where('employee_id', $r['user_id'])->where('clock_out =', '')->first();
				// clock in time
				$clock_in_time = strtotime($attendance['clock_in']);
				$fclock_in = date("H:i:s", $clock_in_time);
			...
			$total_hrs = $TimesheetModel->where('employee_id', $r['user_id'])->where('attendance_date', $attendance_date)->where('total_work !=', '')->findAll();
				// $total_hrs = $TimesheetModel->where('employee_id', $r['user_id'])->where('total_work !=', '')->findAll();
				$hrs_old_int1 = 0;
				$Total = '';
				$Trest = '';
				$total_time_rs = '';
			...
			if($check_out_row>0){
				...
				if ($first_out[0]->clock_out!='') {
					$clock_out2 = date("H:i:s", $clock_out);
	...
	public function attendance_list_work() {
		...
		$HrisModel = new HrisModel();
		$ImsModel = new ImsModel(); 
		$TimesheetijinB3 = new TimesheetijinB3Model(); 
		...
		foreach($get_data as $r) {  
			...
			foreach($employee as $e){
				...
				$name=$nama['nama_karyawan']; 
				$no_sim_b2_umum=$nama['no_sim_b2_umum'];  
				$no_sim_b2_umum_expiredate=$nama['no_sim_b2_umum_expiredate'];  
				$file=$nama['file'];    
			} 
			$employee_name = $name;
			...
			$Trest = ''; 
			$b3_ijin = $TimesheetijinB3->where('kode_supir', $r['payroll_id'])->where('tgl_transaksi',date('Y-m-d',strtotime($r['clock_in'])))->first();  
			if(isset($nama['no_sim_b2_umum_expiredate']) && $nama['no_sim_b2_umum_expiredate'] !="")
			{
				$sim_expired = date("d-m-Y", strtotime($nama['no_sim_b2_umum_expiredate']));
			}
			else {
				$sim_expired = "";
		    }
			
			if(isset($b3_ijin['expired_date']) && $b3_ijin['expired_date'] !="")
			{
				$b3_expired = date("d-m-Y", strtotime($b3_ijin['expired_date']));
			}
			else {
				$b3_expired = "";
		    }

			if(isset($b3_ijin['tgl_expired']) && $b3_ijin['tgl_expired'] !="")
			{
				$stnk_expired = date("d-m-Y", strtotime($b3_ijin['tgl_expired']));
			}
			else {
				$stnk_expired = "";
		    }  

			$sim='<a href="http://cilegon.bcs-logistics.co.id/hris/'.$file.'" target="blank">'.$no_sim_b2_umum.'</a> - ('.$sim_expired.')'; 
 			$ijin_b3= '<a href="http://cilegon.bcs-logistics.co.id/dms/uploads/'.$b3_ijin['attach_file'].'" target="blank">'.$b3_ijin['attach_file'].'</a> ('.$b3_expired.')';
			$stnk= '<a href="http://cilegon.bcs-logistics.co.id/fms/uploads/'.$b3_ijin['attachment_3'].'" target="blank">'.$b3_ijin['attachment_3'].'</a> ('.$stnk_expired.')';
	...
	public function attendance_list_not_work() {  	 
		...
		foreach($get_data as $r) {
			...
			$b3_ijin = $TimesheetijinB3->where('kode_supir', $r['payroll_id'])->where('tgl_transaksi',date('Y-m-d',strtotime($r['clock_in'])))->first();  
			if(isset($no_sim_b2_umum_expiredate) && $no_sim_b2_umum_expiredate !="")
			{
				$sim_expired = date("d-m-Y", strtotime($no_sim_b2_umum_expiredate));
			}
			else {
				$sim_expired = "";
		    }
...
// 		echo $this->request->getGet('user');
// 		echo $this->request->getGet('date');
// 		die();
// 		exit();
// 		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
// 		if($user_info['user_type'] == 'staff'){
...
// 		}
		...
		if ($this->request->getGet('user')!='0') {
			...
			$get_data = $TimesheetModel->where('attendance_date',$date)->findAll();
		}
		...
		$data = array();
          ...
          foreach($get_data as $r) {						
			...
			$fclckIn = date("H:i:s", $clock_in_time);
			
// 			$clock_out_time = strtotime($r['clock_out']);
// 			$fclckOut = date("H:i:s", $clock_out_time);
			$attendance_date = set_date_format($r['attendance_date']);	
			
			
            // Check if clock_out is not empty
            if (!empty($r['clock_out'])) {
                $clock_out_time = strtotime($r['clock_out']);
                $fclckOut = date("H:i:s", $clock_out_time);
            } else {
                $fclckOut = '-'; // or you can set it to an empty string ''
            }
	...
	public function set_clocking() {		
		...
		$request = \Config\Services::request(); 
		$TimesheetModel = new TimesheetModel();
		$TimesheetWorkModel = new TimesheetWorkModel();
		...
		if ($this->request->getPost('type') === 'set_clocking') {
			...
			if($clock_state=='clock_in') {
				...
				//  echo $id.'___';
				$datanya 	= $TimesheetModel->where('time_attendance_id', $id)->first(); 
				$data_twork = $TimesheetWorkModel->where('time_attendance_id', $id)->first(); 
 				$clock_in 	= $datanya['clock_in']; 
 				$total_work2 = $data_twork['total_work2']; 

				//  echo $clock_in.'__________'.$nowtime; 
				// echo date("H:i:s",strtotime($clock_in));
				// echo date("H:i:s",strtotime($nowtime));
				// $difference = abs(strtotime($nowtime) - strtotime($clock_in))/3600;
				// echo $difference;
				// echo  strtotime($difference);echo date("H:i:s", strtotime($difference))."\n";
				...
				// $hours_in   = $interval_cin->format('%h');
				// $minutes_in = $interval_cin->format('%i');	 
				//  echo $clock_in->diff(date('Y-m-d H:i:s'));  
				// return $total_work;
				$total_work=$total_work2; 
				$data = array(
							'employee_id' => $employee_id,
							'clock_out' => $nowtime,
