...
class Warning extends BaseController {
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if(!$this->validate($rules)){
				$ruleErrors = [
					...
					"warning_date_to" => $validation->getError('warning_date_to'),
				...
				$warning_date_to = $this->request->getPost('warning_date_to',FILTER_SANITIZE_STRING);
	...
	public function warning_list() {
		...
		$get_data = $WarningModel->orderBy('warning_id', 'ASC')->findAll();
		...
		$data = array();
          ...
          foreach($get_data as $r) {						
			...
			$nama=$HrisModel->where('payroll_id',$warning_to['payroll_id'])->first();
			$namanya=$nama['nama_karyawan'];
			$name = $warning_to['payroll_id'].' - '.$namanya;
			...
			$warning_date_to = set_date_format($r['warning_date_to']);
	...
	// record list
	public function warning_list_dash() {
		...
		$user_info = $UsersModel->where('user_id', $usession['sup_user_id'])->first();
		if($user_info['user_type'] == 'staff'){
			$get_data = $WarningModel->where('warning_to',$user_info['user_id'])->where('warning_date <=',date('Y-m-d'))->where('warning_date_to >=',date('Y-m-d'))->orderBy('warning_id', 'ASC')->findAll();
		} else {
			$get_data = $WarningModel->where('company_id',$usession['sup_user_id'])->where('warning_date <=',date('Y-m-d'))->where('warning_date_to >=',date('Y-m-d'))->orderBy('warning_id', 'ASC')->findAll();
		}
		$data = array();
	...
	{ 
		...
		$xin_system = $SystemModel->where('setting_id', 1)->first();
		$data['title'] = lang('Dashboard.left_warnings').' | '.$xin_system['application_name'];
		$data['path_url'] = 'warning_dash';
		$data['breadcrumbs'] = lang('Dashboard.left_warnings');

		$data['subview'] = view('erp/disciplinary_cases/key_disciplinary_cases_dash', $data);
		return view('erp/layout/layout_main', $data); //page load
	}
}
